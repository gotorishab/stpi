...
class Website(Home):
    ...
    @http.route('/blog/<model("blog.post"):post>/upvote', type='json', auth="public", website=True)
    def blog_upvote(self, post, **kwargs):
    ...
    @http.route('/blog/<model("blog.post"):post>/downvote', type='json', auth="public", website=True)
    def blog_downvote(self, post, **kwargs):
        if not request.session.uid:
            return {'error': 'anonymous_user'}
        if request.uid == post.create_uid.id:
            return {'error': 'own_post'}
