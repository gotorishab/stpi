...
class PurchaseOrderLine(models.Model):
    ...
    stock_request_ids = fields.Many2many('stock.request',string='Stock Requests', copy=False)

    @api.multi
    def _prepare_stock_moves(self, picking):
        # print("========================== in POL _prepare_stock_moves==============",picking)
        res = super(PurchaseOrderLine, self)._prepare_stock_moves(picking)
        for re in res:
            re['allocation_ids'] = [(0, 0, {'stock_request_id': request.id,
                                            'requested_product_uom_qty': request.product_qty,
                                            }) for request in self.stock_request_ids]
        # print("======================= final res going to return====================")
        return res
