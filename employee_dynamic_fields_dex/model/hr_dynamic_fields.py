...
#         print("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<",field_list)
        ...
        if ('serialized', 'serialized') in field_list:
    ...
    def _set_default(self):
        model_id = self.env['ir.model'].sudo().search([('model', 'in', ('hr.employee','hr.contract'))])  
        return [('id', 'in', model_id.ids)]
...
#                                                    'copy': self.copy,
        ...
        if self.model_id.model ==  'hr.employee':
            ...
            d = {'name': 'hr.employee.dynamic.fields',
                  ...
                  'active': True}
#             print"<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<",d
        elif self.model_id.model == 'hr.contract':
#
...
#                                      domain=set_domain1, required=True)
    ...
    ref_model_id = fields.Many2one('ir.model', index=True)
