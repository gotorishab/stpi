...
class ExceptionRule(models.Model):
    ...
    rule_group = fields.Selection(
        selection_add=[('appraisal_main','Appraisal Manager'),
                       ],
    )
    model = fields.Selection(
        selection_add=[
            ('appraisal.main', 'Appraisal Manager')
        ])

class AppraisalComplete(models.Model):
    ...
    def button_completed(self):
        if self.detect_exceptions():
            ...
            return super(AppraisalComplete, self).button_completed()

    @api.model
    def _get_popup_action(self):
        action = self.env.ref('hr_exception.action_appraisal_main_hod_confirm')
        return action
    ...
    def button_reject(self):
        ...
        return super(AppraisalComplete, self).button_reject()
    ...
    def approve(self):
        ...
        if res:
            ...
            if self.model_id.model == 'appraisal.main':
                self.resource_ref.button_completed()
        return res
