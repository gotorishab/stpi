...
class HrHolidaysPublic(models.Model):
    ...
    def allow_public_holiday_on_caledar(self):
        for public in self:
            for line in public.line_ids:
                leave_cre = self.env['hr.leave'].create({
                                                'holiday_type':'branch',
                                                'by_branch_id':public.branch_id.id,
    ...
    def _compute_display_name(self):
        for line in self:
            if line.country_id:
                line.display_name = '%s (%s)' % (line.year,line.country_id.name
                )
            else:
...
class HrHolidaysPublicLine(models.Model):
    ...
    def allow_public_holiday_on_caledar(self):
        for public in self:
            employee_ids = self.env['hr.employee'].search([('branch_id','=',public.branch_id.id)])
