...
class LateComingReport(models.Model):
    ...
    epmloyee_contribution = fields.Char('CPF')
    voluntary_contribution = fields.Char('CEPF + VCPF')
    employer_contribution = fields.Char('Employer (C)')
    interest_employee_voluntary = fields.Char('Interest (CPF)')
    interest_employer = fields.Char('Interest (CEPF + VCPF)')
