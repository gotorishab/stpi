...
class WizardLateComing(models.TransientModel):
    ...
    @api.multi
    def confirm_report_by_branch(self):
        for rec in self:
            X = rec.interest_rate
            print('=============X===============', X)
            dr = self.env['pf.ledger.report'].search([('branch_id', 'in', rec.branch_ids.ids),('ledger_for_year', '=', rec.ledger_for_year.id)])
            for lines in dr:
                lines.unlink()

            pay_rules_old = self.env['pf.employee.details'].search(
                    [('pf_details_id.employee_id.branch_id', '=', rec.branch_ids.ids),
                     ('date', '<', rec.ledger_for_year.date_start)
                     ])
            total = 0.00
            for ln in pay_rules_old:
